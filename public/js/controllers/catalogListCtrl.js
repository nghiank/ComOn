		...
		$scope.prepareSearchString = function (copy) {
			...
			function gatherOR() {
				function getLeft(orExpStartIndx) {
					...
					if (opLeft[opLeftLen - 1] === ('"')) // This is a quoted expression, potentially
					{
						opLeftLen -= 1; // We are indexing...
						while (--opLeftLen >= 0)
						{
							if (opLeft[opLeftLen] === '"' &&
								(opLeftLen === 0 || opLeft[opLeftLen - 1] === ' '))
							{
				...
				function getRight(orExpStartIndx) {
					...
					var opRightLen = 0;
					if (opRight[0] === ('"')) // Start of a quoted exact phrase
					{
						...
						{
							if (opRight[opRightLen] === '"' &&
								(opRightLen === opRight.Length - 1 || opRight[opRightLen + 1] === ' '))
							{
			...
			function cleanSearchString() {
				...
				temp = $scope._.sortBy(temp, function(val) {return val? val.length: 0;});
